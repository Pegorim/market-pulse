...
assert.match(state.status(q.A,5000), /antiga/);
...
assert.equal(q.B.price,null); assert.match(state.status(q.B,6000), /Indisponível/);
...
assert.equal(state.session(q.A),'Sessão não informada');
assert.equal(state.session({marketState:'CLOSED'}),'Mercado fechado');
